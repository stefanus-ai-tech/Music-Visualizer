// ============================================================
// N3 — MASHLE "ERRATIC STROBE" ORGANISM
// ============================================================
...
class ImpactGraphic {
    ...
    draw(ctx) {
        ...
        // Make impacts contrast with the chaotic background
...
startBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        analyser.smoothingTimeConstant = 0.5; // Very raw for sharp spikes
...
function renderLoop() {
    requestAnimationFrame(renderLoop);
    time += 0.03; // Slightly faster global time
    frameCounter++;
 ...
 // THE BEAT DROP LOGIC
    // Turunin syarat selisih bass (delta) dari 0.15 ke 0.08
    if (eB - lastBass > 0.08 && eB > spawnThreshold) {
        beatTimer = 1.0; 
        
        currentScale = 1.3 + (eB * 0.4); 
    ...
    const shakeInt = ctrlShake ? parseFloat(ctrlShake.value) : 0.8;
    if (beatTimer > 0.3 && shakeInt > 0) {
        const shakeMag = beatTimer * 60 * shakeInt;
...
function renderBackground() {
    ...
    const globalRot = time * 0.005 * (ctrlRotation ? parseFloat(ctrlRotation.value) : 0.5);
    ...
    for (let i = 0; i < bgElements.length; i++) {
        ...
        // Adjust element colors so they don't vanish into the background
        if (el.type === 'text') {
            ...
            ctx.fillStyle = currentBgColor; // Crescent cutout matches current BG
            ctx.fill();
        }
...
function renderErraticCore() {
    ...
    const maxSpike = Math.min(canvas.width, canvas.height) * 0.45; 
    ...
    for (let i = 0; i < BINS; i++) {
        ...
        targetRadii[i] = baseRadius + (normalized * maxSpike * (1 + bassEnergy * 2.0));
        snappedRadii[i] += (targetRadii[i] - snappedRadii[i]) * 0.5; // Aggressive snap
    }

    const drawJagged = (scale = 1, yOffset = 0) => {
        ctx.beginPath();
        for (let i = 0; i < BINS; i++) {
            const angle = (i / BINS) * Math.PI * 2 + (time * 0.1 * (ctrlRotation ? parseFloat(ctrlRotation.value) : 1));
            
            // Jitter for that erratic, glitchy feel on the beat
            const jitter = (Math.random() - 0.5) * beatTimer * 0.15;
            
            let r = snappedRadii[i] * scale * (1 + jitter);
            
            // Alternate spikes for sharpness (Star/sawtooth effect)
            if (i % 2 !== 0) {
                r *= 0.6; // Inner cut
            }
            ...
            else ctx.lineTo(x, y); // LINETO instead of curves for sharp edges
        }
        ctx.closePath();
    };
    ...
    const splitDistance = bassEnergy * 100 + (beatTimer * 60); 
    
    // We add a thick white stroke underneath everything so it pops on black/orange backgrounds
    const drawWithOuterStroke = (drawFunc, fillColor) => {
        drawFunc();
        // Thick protective white border
        ctx.lineWidth = 14;
        ctx.strokeStyle = COLOR_WHITE;
        ctx.stroke();
        
        // Inner black heavy ink line
        ctx.lineWidth = 6;
