...
function resizeCanvas() {
    ...
    cols = clamp(Math.floor(targetCells * (W / Math.max(W, H)) * 1.65), 44, 150);
    rows = clamp(Math.floor(targetCells * (H / Math.max(W, H)) * 1.65), 30, 100);
...
function seedField() {
    ...
    const count = Math.floor(clamp(W * H / (1500 * dpr), 520, 1600));
...
function drawField() {
    ...
    const bg = ctx.createRadialGradient(W * 0.5, H * 0.42, 0, W * 0.5, H * 0.5, Math.max(W, H) * 0.76);
    bg.addColorStop(0, rgba(pal.paper, 0.92));
    bg.addColorStop(0.55, rgba(pal.bg, 0.82));
    ...
    const step = Math.max(1, Math.floor(cols / 92));
    for (let y = 0; y < rows; y += step) {
        for (let x = 0; x < cols; x += step) {
            ...
            const c = pal.colors[Math.floor(clamp((pressure[i] + curl[i] * 0.2) * 2.2, 0, pal.colors.length - 1)) % pal.colors.length];
            const alpha = clamp(0.025 + d * 0.25 + speed * 0.15, 0.02, 0.58) * intensity;
            const size = Math.max(1, cell * 0.62 * step * (0.72 + d * 0.8 + beatPulse * 0.2));
            ...
            if ((x + y) % (step * 5) === 0) {
                ctx.strokeStyle = rgba(pal.paper, alpha * 0.42);
                ctx.lineWidth = Math.max(0.4, dpr * 0.55);
                ctx.beginPath();
                ctx.moveTo(px, py);
                ctx.lineTo(px + ux[i] * cell * 4.6 * thickness, py + uy[i] * cell * 4.6 * thickness);
                ctx.stroke();
            }
        }
    }
...
function updateParticles() {
    ...
    for (const p of particles) {
        ...
        ctx.strokeStyle = rgba(p.color, clamp(0.08 + audio * 0.5, 0.08, 0.62) * p.life);
...
function drawPressureContours() {
    ...
    ctx.lineWidth = Math.max(0.35, dpr * 0.55);
    const stride = Math.max(3, Math.floor(cols / 34));
    for (let y = stride; y < rows - stride; y += stride) {
        ...
        ctx.strokeStyle = rgba(pal.bg, 0.035 + midEnergy * 0.03);
        ctx.stroke();
    }
    ctx.restore();
}
